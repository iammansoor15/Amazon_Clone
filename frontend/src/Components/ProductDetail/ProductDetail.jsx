  ...
  return (
    <>
      <div className={styles.mainContainer}>
        <div className={styles.imageContainer}>
          <div className={styles.allImages}>
            {images.map((img, index) => (
              <div className={`${styles.exampleImages} ${selectedImage === img ? styles.selected : ""}`}
                key={index}
                onClick={() => {
                  setMainImage(img);
        ...
        <div className={`${styles.textContainer} flex my-10`}>
          <div className={`${styles.descContainer} min-w-3/4 px-10`}>
            ...
            <span className="bg-red-600 px-2 py-1 text-white text-xs font-bold">Limited time deal</span>
            <div className="flex">
              <span className="text-2xl font-light">-69%</span>
              <span className="text-2xl font-medium px-3">₹{product.Price[0].deal_price}</span>
            </div>

            <div className="flex">
              <span className="text-xs font-normal">M.R.P :</span>
              <span className="text-xs line-through font-">₹{product.Price[0].org_price}</span>
            </div>
            ...
            <div className="my-5">
              ...
              <ul className="ml-5 list-disc">
                {product.desc.map((item, ind) => (
                  <li key={ind} className="mb-2">{item}</li>  
                ))}
              </ul>
            </div>
          </div>

          <div className={`${styles.orderContainer} pr-10`}>
            ...
            <button onClick={()=>{addToCart(id)}} className="bg-yellow-400 w-full px-4 py-1 rounded-full my-2 mt-20">
              Add to cart
            </button>
            <button  onClick={() => buyItem(id)} className="bg-yellow-400 w-full px-4 py-1 rounded-full">
              Buy Now
            </button>
          </div>
        </div>
