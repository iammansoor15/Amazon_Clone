  ...
  const deleteItem = async (id) => {
    try {
      ...
      if (res.ok) {
        ...
        setAccount((prev) => ({...prev,cart: prev.cart.filter((item) => item.id !== id),}));
        fetchitems();
      } else {
  ...
  return (
    <div className={styles.screen}>
          ...
          cartData.map((item, ind) => (
            <div className={styles.itemDesc} key={ind}>
              ...
              <div className={styles.descText}>
                <NavLink to={`/productdetail/${item.id}`}>
                  <h3 className="text-xl ">{item.ItemTitle}</h3>
      ...
      <div className={styles.amount}>
        ...
        <button onClick={selectedItems.length > 0 ? buyItemsFromCart : undefined}
